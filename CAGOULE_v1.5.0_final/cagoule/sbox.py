...
# Désactivation de la S-box cubique pour les grands premiers
# Le fallback x^d est utilisé pour p >= _EXHAUSTIVE_THRESHOLD (100)
# Ce choix est volontaire : la vérification exhaustive devient trop coûteuse
# et les propriétés de bijectivité ne sont pas garanties pour les grands p.
_USE_CUBIC_SBOX_FOR_LARGE_PRIMES = False  # Documentaire (non utilisé directement)
...
def verify_sbox_bijective(c: int, p: int) -> bool:
    ...
    Note importante : Pour p >= _EXHAUSTIVE_THRESHOLD (100), cette vérification
    est désactivée et retourne False. La S-box cubique n'est donc utilisée que
    pour les petits premiers. Pour les grands premiers (cas réel en production,
    où p ≈ 2⁶⁴), le fallback x^d est systématiquement employé.
    """
    if p == 2:
        # x³ + c*x : x=0→0, x=1→1+c
...
def find_valid_c(delta: int, p: int, max_attempts: int = _FIND_C_MAX_ATTEMPTS):
    ...
    Note : Pour p >= _EXHAUSTIVE_THRESHOLD (100), retourne directement (None, -1)
    car la vérification exhaustive est désactivée. En production, avec p ≈ 2⁶⁴,
    cette fonction ne sera donc jamais utilisée.
    """
    # Pour les grands p, fallback systématique
    if p >= _EXHAUSTIVE_THRESHOLD:
        return None, -1
