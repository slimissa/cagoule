"""
matrix.py — Matrice de diffusion CAGOULE v2.3.0
...
# Backend query (v2.2.0)
def _query_matrix_backend() -> str:
    """Retourne 'avx2' si le backend AVX2 est actif, 'scalar' sinon."""
    if not CAGOULE_C_AVAILABLE or _lib is None:
        return 'python'
    try:
        fn = _lib.cagoule_matrix_backend_is_avx2
        fn.restype = ctypes.c_int
        return 'avx2' if fn() else 'scalar'
    except Exception:
        return 'scalar'
...
class DiffusionMatrixC:
    ...
    def __init__(self, ptr: ctypes.POINTER, p: int, kind: int) -> None:
        ...
        self.n = CAGOULE_N
        self._freed = False  # guard pour double-free (v2.2.0)
    ...
    # ── v2.2.0 : gestion mémoire déterministe ───────────────────────────

    def free(self) -> None:
        """
        Libère la mémoire C de façon déterministe.

        Préférer free() ou le context manager plutôt que __del__ pour les
        applications long-durée (daemons, serveurs) afin d'éviter les
        problèmes d'ordre de destruction en fin de programme.

        Lève RuntimeError si appelé deux fois.
        """
        if self._freed:
            raise RuntimeError(
                "DiffusionMatrixC.free() appelé deux fois — double-free interdit"
            )
        self._freed = True
        if self._ptr is not None and _lib is not None:
            try:
                _lib.cagoule_matrix_free(self._ptr)
            except Exception as e:
                _log.debug(f"free(): erreur libération matrice: {e}")
            finally:
                self._ptr = None

    def __enter__(self) -> "DiffusionMatrixC":
        """Context manager — garantit free() même en cas d'exception."""
        return self

    def __exit__(self, *_) -> None:
        """Libère la mémoire C à la sortie du bloc with."""
        if not self._freed:
            self.free()

    def __del__(self) -> None:
        """Fallback de sécurité — préférer free() ou le context manager."""
        if not self._freed and self._ptr is not None and _lib is not None:
            try:
                _lib.cagoule_matrix_free(self._ptr)
                self._ptr = None
            except Exception as e:
                _log.debug(f"__del__: erreur libération matrice: {e}")

    @property
    def backend_info(self) -> str:
        """Retourne le backend actif : 'avx2' ou 'scalar' (v2.2.0)."""
        return _query_matrix_backend()
