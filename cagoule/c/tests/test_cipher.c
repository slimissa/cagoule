/**
 * test_cipher.c — Tests du pipeline CBC CAGOULE v2.3.0
 *
 * Usage :
...
static void test_avx2_parity(void) {
    ...
    /* AVX2 path (default) */
    cagoule_cbc_encrypt(plaintext, 1, ct_avx2, ct_size,
                        g_mat, &g_sbox, g_round_keys, NUM_KEYS, P);
    
    /* Verify decrypt works */
    cagoule_cbc_decrypt(ct_avx2, 1, recovered, 16,
                        g_mat, &g_sbox, g_round_keys, NUM_KEYS, P);
    CHECK(memcmp(plaintext, recovered, 16) == 0, "AVX2 encrypt/decrypt roundtrip");
    
    /* Compare against known scalar output from a KAT vector, or:
     * just verify the output is non-trivial */
    int changed = 0;
    for (int i = 0; i < (int)ct_size; i++)
        if (ct_avx2[i] != 0) { changed = 1; break; }
    CHECK(changed, "AVX2 ciphertext non-nul");
}
#endif
...
int main(void) {
    printf("══════════════════════════════════════════\n");
    printf("  CAGOULE v2.3.0 — test_cipher.c\n");
    printf("══════════════════════════════════════════\n");
