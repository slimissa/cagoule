...
static void test_avx2_scalar_parity(void) {
    ...
    uint64_t v[CAGOULE_N], out_avx2[CAGOULE_N], out_scalar[CAGOULE_N];
    for (int i = 0; i < CAGOULE_N; i++)
        v[i] = (uint64_t)((i * 7919) % P_BENCH);
    
    /* AVX2 path (default if available) */
    cagoule_matrix_mul(m, v, out_avx2);
    
    /* Explicit scalar path */
    cagoule_matrix_mul_scalar(m, v, out_scalar);
    
    int eq = memcmp(out_avx2, out_scalar, CAGOULE_N * 8) == 0;
    CHECK(eq, "AVX2 == Scalar bit-for-bit");
...
static void bench_matmul(void) {
    ...
    if (!under_valgrind) {
        printf("  [v2.3.0 AVX2 target: ~80 ms forward, ~80 ms inverse]\n");
...
int main(void) {
    printf("══════════════════════════════════════════\n");
    printf("  CAGOULE v2.3.0 — test_matrix.c\n");
    printf("══════════════════════════════════════════\n");
