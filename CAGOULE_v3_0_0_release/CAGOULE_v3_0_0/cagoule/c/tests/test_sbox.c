...
static void test_feistel_symmetry(void) {
    ...
    for (int i = 0; i < N; i++)
        r = cagoule_sbox_forward(&s, (uint64_t)i);
    double fwd_ms = (double)(clock() - t0) / CLOCKS_PER_SEC * 1000.0;

    t0 = clock();
    for (int i = 0; i < N; i++)
        r = cagoule_sbox_inverse(&s, (uint64_t)i);
    ...
    printf("  [Objectif v2.3.0   : ratio < 1.5×]\n");

    int ratio_ok = (fwd_ms < 0.5) || (inv_ms / fwd_ms < 1.5);
    CHECK(ratio_ok, "ratio inv/fwd proche de 1× (propriété Feistel, < 1.5×)");
...
int main(void) {
    printf("══════════════════════════════════════════\n");
    printf("  CAGOULE v3.0.0 — test_sbox.c\n");
    printf("══════════════════════════════════════════\n");
