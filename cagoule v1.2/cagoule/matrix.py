...
Requis par : cipher.py, decipher.py, params.py
"""
...
def matrix_vec_mul_mod(m: Matrix, v: list[int], p: int) -> list[int]:
    """Produit matrice-vecteur M × v mod p."""
    n = len(m)
    result = []
    for i in range(n):
...
class DiffusionMatrix:
    ...
    def apply(self, block: list[int]) -> list[int]:
        ...
        return matrix_vec_mul_mod(self.matrix, block, self.p)
    ...
    def apply_inverse(self, block: list[int]) -> list[int]:
        ...
        return matrix_vec_mul_mod(self.matrix_inv, block, self.p)
