"""
omega.py — Pilier Ω : ζ(2n) → Round Keys — CAGOULE v2.5.0
...
# ── Backend actif ────────────────────────────────────────────────────────────
OMEGA_BACKEND: str = (
    "C (libcagoule.so v2.5.0)"
    if _OMEGA_C_SYMBOLS_OK
    else "Python (mpmath fallback)"
)

# ── Chargement conditionnel de mpmath ────────────────────────────────────────
# CORRECTIF v3.0.1 : mpmath était importé uniquement si _OMEGA_C_SYMBOLS_OK==False,
# rendant _mpmath_available=False sur toute build normale avec libcagoule.so présent.
# Les tests bit-exacts C↔Python (TestBitExactCompatibility) ne pouvaient donc jamais
# s'exécuter — fausse couverture. Fix : importer inconditionnellement.
...
try:
    ...
    # mpmath absent mais C disponible — fonctionnement nominal, tests croisés désactivés
