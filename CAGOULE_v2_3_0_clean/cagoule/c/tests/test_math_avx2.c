...
static void m256i_to_u64(const __m256i v, uint64_t out[4]) {
    _mm256_storeu_si256((__m256i*)out, v);  // Store directly to aligned output
}
#endif
...
#ifndef __AVX2__
    printf("      SKIP — AVX2 non disponible\n");
    _pass++;
    return;
#else
...
static void test_edge_cases(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int k = 0; k < 4; k++) {
            if (got[k] != exp) {
                fprintf(stderr, "  FAIL  edge mulmod a=%llu b=%llu lane=%d got=%llu exp=%llu\n",
                        (unsigned long long)a, (unsigned long long)b,
                        k, (unsigned long long)got[k], (unsigned long long)exp);
                _fail++;
            } else {
                _pass++;
            }
...
int main(void) {
    printf("══════════════════════════════════════════════════\n");
    printf("  test_math_avx2 — CAGOULE v2.3.0\n");
    printf("══════════════════════════════════════════════════\n");
