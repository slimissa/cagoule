                                  ...
                                  __m256i p_vec, uint64_t mu)
{
    ...
    /* Étape 4 : replier r_hi via 2^64 ≡ (2^64 - p) mod p
     *
     * Pour chaque unité de r_hi, ajouter mod2_64 à r.
     * CORRECTION DE DÉBORDEMENT : si r + mod2_64 déborde uint64
     * (r_new < r_prev unsigned), ajouter mod2_64 une fois de plus.
     * La seconde addition ne déborde jamais (preuve dans l'en-tête).
     */
    __m256i mask1 = _cmpgt_epu64(r_hi, zero);   /* r_hi >= 1 */
    if (!_mm256_testz_si256(mask1, mask1)) {
        __m256i add1  = _mm256_and_si256(mask1, mod2_64);
        __m256i r_old = r;
        r = _mm256_add_epi64(r, add1);
        /* Débordement uint64 : r < r_old (unsigned) → +mod2_64 encore */
        __m256i ovf1 = _mm256_and_si256(_cmpgt_epu64(r_old, r), mask1);
        r = _mm256_add_epi64(r, _mm256_and_si256(ovf1, mod2_64));
    }

    __m256i mask2 = _cmpgt_epu64(r_hi, one);    /* r_hi >= 2 */
    if (!_mm256_testz_si256(mask2, mask2)) {
