"""test_kat.py — Known Answer Tests CAGOULE v2.3.0
...
class TestKATParameters:
    ...
    def test_version(self, kat):
        """La version du KAT doit être 2.0, 2.1.0, ou 2.2.0."""
        assert kat["version"] in ["2.0", "2.1.0", "2.2.0", "2.3.0"], f"Version KAT incorrecte: {kat['version']}"
...
class TestKATNonRegression:
    ...
    def test_sha256_fixed(self, kat):
        """L'empreinte SHA-256 du CGL1 KAT ne doit pas changer (v2.3.0)."""
        # v2.3.0 : S-box AVX2 → nouveau vecteur KAT
        expected_sha = "ae7c711ffdf88316a6c6211d9f0b57df0e266e196379746b86f20e5ccab513fc"
        actual_sha = kat["output"]["sha256_cgl1"]
        if not actual_sha:
            pytest.skip("sha256_cgl1 non disponible")
        # Ignorer les empreintes des versions antérieures
        if kat["version"] in ("2.1.0", "2.2.0"):
            pytest.skip(f"Version {kat['version']} - empreinte différente de v2.3.0")
