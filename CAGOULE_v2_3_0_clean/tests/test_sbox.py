"""test_sbox.py — Tests S-box CAGOULE v2.3.0 (Feistel + fallback x^d)"""
...
class TestSBoxFallback:
    ...
    def test_repr_fallback(self, p):
        """La représentation doit indiquer le mode fallback (v2.1.0+).
        Accepte aussi le format C (v2.0+).
        """
        s = _sbox(p)
        rep = repr(s)
        assert (
            "fallback" in rep.lower() or
            "x^d" in rep or
            "Python" in rep or
            ("C" in rep and "x^" in rep)
        ), f"repr inattendu: {rep!r}"
