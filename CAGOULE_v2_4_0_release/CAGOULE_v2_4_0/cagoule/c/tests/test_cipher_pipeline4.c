/**
 * test_cipher_pipeline4.c — Tests parité & benchmarks pipeline4 v2.4.0
 *
 * Vérifie :
...
static void test_guards(void) {
    ...
    CHECK(cagoule_cbc_encrypt(NULL, 1, out, csz, g_mat, &g_sbox, g_rk, NUM_KEYS, P)
          == CAGOULE_ERR_NULL, "encrypt NULL padded");
    CHECK(cagoule_cbc_encrypt(plain, 1, NULL, 0, g_mat, &g_sbox, g_rk, NUM_KEYS, P)
          == CAGOULE_ERR_NULL, "encrypt NULL out");
    CHECK(cagoule_cbc_encrypt(plain, 4, out, 0, g_mat, &g_sbox, g_rk, NUM_KEYS, P)
          == CAGOULE_ERR_SIZE, "encrypt out_size=0");
    CHECK(cagoule_cbc_decrypt(NULL, 1, plain, sizeof(plain), g_mat, &g_sbox, g_rk, NUM_KEYS, P)
          == CAGOULE_ERR_NULL, "decrypt NULL cipher");
    CHECK(cagoule_cbc_decrypt(out, 4, NULL, 0, g_mat, &g_sbox, g_rk, NUM_KEYS, P)
...
static void test_parity_random(void) {
    ...
    for (int iter = 0; iter < 10000; iter++) {
        ...
        int r1 = cagoule_cbc_encrypt(plain, nb, ct1, ct_sz, g_mat, &g_sbox, g_rk, NUM_KEYS, P);
        int r2 = cagoule_cbc_encrypt(plain, nb, ct2, ct_sz, g_mat, &g_sbox, g_rk, NUM_KEYS, P);
        int r3 = cagoule_cbc_decrypt(ct1, nb, pt, plain_sz, g_mat, &g_sbox, g_rk, NUM_KEYS, P);
...
static void test_benchmark(void) {
    ...
    /* Warmup */
    cagoule_cbc_encrypt(plain, nb, ct, ct_sz, g_mat, &g_sbox, g_rk, NUM_KEYS, P);
    ...
    uint64_t t1 = now_ms();
    for (size_t r = 0; r < n_runs; r++)
        cagoule_cbc_decrypt(ct, nb, pt, plain_sz, g_mat, &g_sbox, g_rk, NUM_KEYS, P);
...
int main(void) {
    printf("═══════════════════════════════════════════════════════\n");
    printf("  test_cipher_pipeline4 — CAGOULE v2.4.0\n");
    printf("═══════════════════════════════════════════════════════\n");
