/**
 * cagoule_matrix.c — Matrice de diffusion Vandermonde CAGOULE v2.4.0
 *
 * Nouveautés v2.2.0 :
...
static int _avx2_available(void) {
    ...
    if (state == 0) {
#if defined(__AVX2__)
        /* Permet à "make test-scalar" de forcer le chemin scalaire */
        ...
        return avx2 ? 1 : 0;   /* v2.3.0: normalize — __builtin_cpu_supports returns bitmask */
    }
    return state == 1;
}
