"""
CAGOULE v2.2.0 — Cryptographie Algébrique Géométrique par Ondes et Logique Entrelacée
...
Inspection du backend :
    ...
    print(__backend__)       # "C (libcagoule.so v2.2)" ou "Python pur (fallback v1.x)"
    print(__omega_backend__) # "C (libcagoule.so v2.2)" ou "Python (mpmath fallback)"
...
__backend__ = "C (libcagoule.so v2.2)" if _C_AVAILABLE else "Python pur (fallback v1.x)"

# Backend info (v2.2.0)
try:
    from ._binding import get_backend_info as _get_backend_info
    backend_info = _get_backend_info()
except Exception:
    backend_info = {"matrix_backend": "unknown", "omega_backend": "unknown"}
